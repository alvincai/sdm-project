...
def main():
    ...
    del ct['C']['C']
    ctPg = objectToBytes(ct, proxy.group)       # type of ctPG is PairingGroup. Use objectToBytes API
    db = Database()
    db.insertRecord(Alice.General[0], ctI, ctPg)
    ...
    ctReconstructed = list()    # List of Ciphertexts
    for row in rows :
        ctI_bytes = bytes(row[0], 'utf-8')
        ctI_Reconstruct = deserialize(ctI_bytes)
        ctPg_bytes = bytes(row[1], 'utf-8')
        ct_Reconstruct = bytesToObject(ctPg_bytes, proxy.group)
        ct_Reconstruct['C']['C'] = ctI_Reconstruct
        ctReconstructed.append(ct_Reconstruct)
    # TODO: Might need to be careful that copy by value and not reference


    pt = Alice.dec("General",ctReconstructed[0])
    print (pt)
